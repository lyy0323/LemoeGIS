...
def index():
    ...
    for st in sts:
        st_info = st.split('\t')
        popup_html = f'''<div align="center" padding: 20px style="font-size: 1.6em">
    ...
    filenames = os.listdir('./static/map_lines')
    # for _ in range(len(filenames)):
    #     # names = locals()
    #     # names['filename%s' % _] = filenames[_]
    #
    #     # def style_fun(feature):
    #     #     names = locals()
    #     #     names['filename%s' % _] = filenames[_]
    #     #     return {"color": metro_color_dict[eval('filename%s' % _).strip('.geojson')], "opacity": 0.5}
    #     folium.GeoJson(f'./static/map_lines/{eval("filename%s" % _)}', style_function=style_fun).add_to(m)
    # def style_fun(x):
    #     return {"color": rtx(tuple(x['properties']['color']))}
    # def
    for filename in filenames:
        ...
        folium.GeoJson(open(f'./static/map_lines/{filename}', encoding='UTF-8').read(), popup=folium.GeoJsonPopup(fields=['ends'], aliases=[popup_html], labels=True, style='font-family: Microsoft Yahei'), style_function=lambda x: {"color": "#ffffff", "weight": 6, "opacity": 0.5}).add_to(metro_lines)
        folium.GeoJson(open(f'./static/map_lines/{filename}', encoding='UTF-8').read(), popup=folium.GeoJsonPopup(fields=['ends'], aliases=[popup_html], labels=True, style='font-family: Microsoft Yahei'), style_function=lambda x: {"color": rtx(tuple(x['properties']['color'])), "weight": 3, "opacity": 0.9}).add_to(metro_lines)
        # folium.GeoJson(f'./static/map_lines/{filename}', popup=folium.Popup(popup_html, parse_html=False, max_width=500), style_function=lambda x: {"color": rtx(tuple(x['properties']['color']))}).add_to(metro_lines)
